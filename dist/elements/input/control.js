...
const circle_1 = require("../svg/circle");
...
*/
class Control extends input_1.Input {
    /**
    ...
    */
    static handleMouseOver(event) {
        if (Control.active == null && !base_element_1.BaseElement.disable && event.target.tagName == 'circle') {
    ...
    */
    handleMouseDown(event) {
        if (!base_element_1.BaseElement.disable) {
            event.preventDefault();
    ...
    */
    translate(x, y) {
        ...
        let point = this.constrain(this, new point_1.Point(x, y));
    ...
    */
    get displayAngle() {
        if (this.y <= 0) {
            ...
            return constants_1.TAU - Math.atan2(this.y, this.x);
        }
    }
    /**
    * Returns the angle in radians relative to the origin that the point forms.
    */
    get angle() {
        return (Math.atan2(this.y, this.x) + constants_1.TAU) % constants_1.TAU;
    }
    /**
    ...
    */
    constrainTo(element) {
        ...
        if (element instanceof path_1.Path) {
            throw Error('not implemented');
        }
        else if (element instanceof circle_1.Circle) {
            this.constrain = function (_oldPosition, newPosition) {
    ...
    */
    constrainToX(minX = -Infinity, maxX = Infinity) {
        this.constrain = function (oldPosition, newPosition) {
            return new point_1.Point(newPosition.x, oldPosition.y);
        };
    }
    /**
    ...
    */
    constrainToY() {
        this.constrain = function (oldPosition, newPosition) {
            return new point_1.Point(oldPosition.x, newPosition.y);
        };
    }
}
